 ...
 * Provides executors for different types of tasks.
 */
public class ThreadUtils {
    ...
    private static final ThreadFactory NETWORK_THREAD_FACTORY = new ThreadFactory() {
        private final AtomicInteger mCount = new AtomicInteger(1);
    ...
    private static final Executor NETWORK_EXECUTOR = Executors.newFixedThreadPool(
            Math.max(2, CPU_COUNT / 2),
            NETWORK_THREAD_FACTORY);
    
    private static final Executor LIGHTWEIGHT_EXECUTOR = Executors.newSingleThreadExecutor(
            r -> {
