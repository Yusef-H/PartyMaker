 ...
 *
 * <p>Handles member listing, invitation management, attendance tracking, and member-related
 * operations within groups.
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Member list display and management
 *   <li>Invitation tracking and management
 *   <li>Attendance status tracking
 *   <li>Member search and filtering
 *   <li>Friend management integration
 *   <li>Member statistics and analytics
 * </ul>
 *
...
public class MembersViewModel extends BaseViewModel {
  ...
  public void inviteMember(@NonNull User user) {
    if (!canInviteMembers()) {
      setError(
          "You don't have permission to invite members", NetworkUtils.ErrorType.PERMISSION_ERROR);
      return;
    }
  ...
  public void removeMember(@NonNull User member) {
    if (!canRemoveMembers()) {
      setError(
          "You don't have permission to remove members", NetworkUtils.ErrorType.PERMISSION_ERROR);
      return;
    }
  ...
  private void updateUserPermissions(Group group) {
    ...
    boolean isAdmin = group.getAdminKey() != null && group.getAdminKey().equals(currentUserKey);
    ...
    ThreadUtils.runOnMainThread(
        () -> {
          ...
          String message =
              member.getUsername()
                  + " is now "
                  + (isComing ? "attending" : "not attending")
                  + " the event";
          setSuccess(message);
