...
/**
 * Class for handling application-specific network errors.
 * Provides methods for displaying appropriate error messages and taking recovery actions.
 */
public class AppNetworkError {
     ...
     */
    public static void showErrorMessage(Context context, NetworkUtils.ErrorType errorType, 
                                        String errorMessage, boolean isServerError) {
        if (context == null) return;
        
        String message = NetworkUtils.getErrorMessage(errorType);
        
        // For server connection errors, offer to go to server settings
        if (isServerError && (errorType == NetworkUtils.ErrorType.NO_NETWORK || 
    ...
    private static void showServerErrorDialog(Context context, String message) {
        new AlertDialog.Builder(context)
            ...
            .setPositiveButton("Server Settings", (dialog, which) -> {
     ...
     * @param isServerError Whether the error is related to the server connection
     */
    public static void handleNetworkError(Context context, String error, 
                                         NetworkUtils.ErrorType errorType, boolean isServerError) {
        if (context == null) return;
