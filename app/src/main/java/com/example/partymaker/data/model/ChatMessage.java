...
public class ChatMessage {
  ...
  /**
   * Creates a copy of this ChatMessage for encryption/decryption operations.
   * Useful to avoid modifying the original message during encryption processing.
   * 
  ...
  public ChatMessage createCopy() {
    ...
    copy.messageContent = new HashMap<>(this.messageContent != null ? this.messageContent : new HashMap<>());
    
    return copy;
  }
  
  /**
   * Checks if the message content appears to be encrypted.
   * This is a basic heuristic check based on content patterns.
   * 
  ...
  /**
   * Mark message as encrypted and update metadata.
   * Useful for tracking encryption status.
   */
  ...
  /**
   * Validates that the message is suitable for encryption.
   * Checks for non-empty content and valid group context.
   * 
  ...
  /**
   * Gets a display-friendly summary of the message for logging.
   * Does not include sensitive content.
   * 
   * @return Safe summary string
   */
  public String getLogSafeSummary() {
    return String.format("ChatMessage{key='%s', group='%s', sender='%s', encrypted=%s, hasImage=%s, length=%d}", 
        messageKey != null ? messageKey.substring(0, Math.min(8, messageKey.length())) + "..." : "null",
