...
/**
 * Repository class for handling group data operations. Acts as a mediator between data sources
...
public class GroupRepository {
  ...
  public void getUserGroups(
      String userKey, ResultCallback<List<Group>> callback, boolean forceRefresh) {
        ...
        new FirebaseServerClient.DataCallback<Map<String, Group>>() {
          ...
          public void onSuccess(Map<String, Group> groups) {
            if (groups == null) {
              ...
              callback.onResult(
                  Result.success(new ArrayList<>())); // Return empty list instead of null
              return;
            }
                  ...
                  public void onResult(Result<List<Group>> result) {
                    if (result.isSuccess()
                      ...
                      for (Group group : result.getData()) {
                        ...
                        } else if (group.getFriendKeys() != null
                      ...
                      if (userGroups.isEmpty()) {
                        ...
                        Log.d(
                            TAG,
                            "Found " + userGroups.size() + " groups for user in fallback search");
                        callback.onResult(Result.success(userGroups));
                      }
                    } else {
                      callback.onResult(
                          Result.error(
                              "Failed to retrieve user groups: " + errorMessage,
    ...
    serverClient.getGroups(
        new FirebaseServerClient.DataCallback<Map<String, Group>>() {
          ...
          public void onError(String errorMessage) {
            ...
            Result<List<Group>> errorResult =
                Result.error(errorMessage, NetworkUtils.ErrorType.SERVER_ERROR);
        ...
        new FirebaseServerClient.DataCallback<Group>() {
          ...
          public void onSuccess(Group group) {
            if (group == null) {
              ...
              callback.onResult(
                  Result.error("Group not found", NetworkUtils.ErrorType.CLIENT_ERROR));
              return;
            }
  ...
  public void updateGroup(
      String groupId, Map<String, Object> updates, ResultCallback<Void> callback) {
    if (groupId == null || groupId.isEmpty()) {
      Log.e(TAG, "Invalid groupId: null or empty");
      callback.onResult(Result.error("Invalid group ID"));
      return;
