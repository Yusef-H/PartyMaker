...
/**
 * Manages group encryption keys through Firebase.
 * Handles key distribution, rotation, and member management.
 * 
 * Firebase Structure:
 * /group_keys/
 *   /{groupId}/
...
public class GroupKeyManager {
    ...
    public CompletableFuture<Boolean> createGroupWithEncryption(String groupId) {
        ...
        try {
            ...
            // Save to Firebase
            firebaseRef.child(groupId).setValue(groupKeyData)
    ...
    public CompletableFuture<Boolean> addUserToGroupEncryption(String groupId, String newUserId) {
        ...
        try {
            ...
            firebaseRef.child(groupId).child("members").limitToFirst(1)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        try {
                            ...
                            firebaseRef.child(groupId).updateChildren(updates)
                                .addOnCompleteListener(task -> {
                                    if (task.isSuccessful()) {
                                        Log.i(TAG, "Successfully added user " + newUserId + " to group encryption: " + groupId);
                                        future.complete(true);
                                    } else {
    ...
    public CompletableFuture<Boolean> removeUserAndRotateKey(String groupId, String removedUserId) {
        ...
        try {
            ...
            firebaseRef.child(groupId).child("members").addListenerForSingleValueEvent(
                new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        try {
                            ...
                            updates.put("metadata/version", System.currentTimeMillis() / 1000); // Use timestamp as version
    ...
    public CompletableFuture<Boolean> loadGroupKey(String groupId) {
        ...
        try {
            firebaseRef.child(groupId).child("members").child(currentUserId)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        try {
    ...
    public CompletableFuture<Boolean> deleteGroupEncryption(String groupId) {
        ...
        try {
            firebaseRef.child(groupId).removeValue()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
    ...
    public void initializeForExistingGroup(String groupId) {
        ...
        // Load from Firebase
        loadGroupKey(groupId).thenAccept(success -> {
            if (success) {
