...
/**
 * Group-based message encryption for PartyMaker chat groups.
 * Each group has a shared AES-256 key that all members use.
 * 
 * Architecture:
 * - One AES key per group (shared by all group members)
 * - Keys stored encrypted in Firebase per user
...
public class GroupMessageEncryption {
    ...
    public boolean hasGroupKey(String groupId) {
        return groupKeyCache.containsKey(groupId) || 
               userStorage.contains("group_key_" + groupId);
    }
    ...
    public String getEncryptionStatus() {
        ...
        return String.format("User: %s, Algorithm: %s, Cached Groups: %d", 
            currentUserId, ALGORITHM, cachedGroups);
    }
